...
- we want to treat single verses within the `<l>` element as independent text segments too, not allowing the tagger to span sentences across the verse boundaries (another approach may also be appropriate, though); these elements occur both as nested within `<p>` and within `<quote>`
...
- we want to exclude anything within the `<teiHeader>` and `<front>`-matter (these parts may otherwise also contain some of the above mentioned elements defined as text elements!)
...
```
tag_ud -f TEI_example1.txt -m en -o TEI_example1.conllu
```
...
```
xml2vrt -i text -e front TEI_example1.ann.xml >TEI_example1.vrt
```
...
```
tag_ud -f Simple_poetry1.txt -m cs -o Simple_poetry1.conllu
ann2standoff Simple_poetry1.conllu
standoff2xml Simple_poetry1.txt
xml2vrt Simple_poetry1.ann.xml >Simple_poetry1.vrt
```
