...
2. Using the option `-vt <element_name>` (configuration option `virtual_token`), empty virtual (sub)token elements will be created to carry the annotation attributes of the individual subtokens. The name of the attribute carrying the form can be specified using the additional option `-va <name>` (configuration option `virtual_attribute`, the default is `form`; use underscore `_` to discard the attribute completely).
...
Particular elements can also be skipped, i.e. excluded from the extraction using the option `-e <element_names>` or the configuration option `skip_elements`. These elements may also be nested.
